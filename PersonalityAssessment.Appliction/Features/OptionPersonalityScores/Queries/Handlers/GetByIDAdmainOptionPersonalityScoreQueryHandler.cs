...
namespace PersonalityAssessment.Application.Features.OptionPersonalityScores.Queries.Handlers
{
        ...
        : IRequestHandler<GetOptionPersonalityScoreByIdAdmainQuery, AdmainReadOptionPersonalityScoreDTO>
    {
        ...
        public GetByIDAdmainOptionPersonalityScoreQueryHandler(IRepository<OptionPersonalityScore> repository, IMapper mapper)
        { _repository = repository; _mapper = mapper; }

        public async Task<AdmainReadOptionPersonalityScoreDTO> Handle(GetOptionPersonalityScoreByIdAdmainQuery request, CancellationToken cancellationToken)
        {
            ...
            if (dto == null) throw new NotFoundException($"OptionPersonalityScore with ID {request.id} not found.");
