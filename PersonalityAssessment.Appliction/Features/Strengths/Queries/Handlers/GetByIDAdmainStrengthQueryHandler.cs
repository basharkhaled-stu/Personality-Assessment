...
{
    public class GetByIDAdmainStrengthQueryHandler : IRequestHandler<GetStrengthByIdAdmainQuery, AdmainReadStrengthDTO>
    {
        private readonly IRepository<Strength> _repository;
        private readonly IMapper _mapper;
        public GetByIDAdmainStrengthQueryHandler(IRepository<Strength> repository, IMapper mapper)
        ...
        public async Task<AdmainReadStrengthDTO> Handle(GetStrengthByIdAdmainQuery request, CancellationToken cancellationToken)
        {
            ...
            if (dto == null) throw new NotFoundException($"Strength with ID {request.id} not found.");
